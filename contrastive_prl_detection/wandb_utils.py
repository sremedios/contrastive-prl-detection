...
class Logger:
    ...
    def histogram(self, values, num_bins=64):
        if not self.enabled:
            return None
        return self._wandb.Histogram(np.asarray(values).ravel(), num_bins=num_bins)
...
class VolumeProbe:
    ...
    def render(self, model, device, tau=ct.TAU, anchors_deg=ct.ANCHORS_DEG):
        """Returns (figure, theta array). Caller closes the figure."""
        from .polar_utils import plot_theta_slices

        theta, seg, _ = infer_volume(model, self.mag, self.pha, device, tau,
                                     anchors_deg, tile=self.tile, halo=self.halo)
        y_hat = wrap_theta(theta)
        counts = np.bincount(seg.cpu().numpy().ravel(), minlength=3)

        fig = plot_theta_slices(y_hat, self.slices, overlay=self.prl,
                                show=False, close=False)
        fig.suptitle(f"{self.subj_id}  (withheld)", fontsize=10)

        del theta, seg
        if device.type == "cuda":
            torch.cuda.empty_cache()
        return fig, y_hat, counts
    ...
    for root, is_pos in ((pos_root, True), (neg_root, False)):
        ...
        for subj_id in withheld_ids:
            if not (root / subj_id).is_dir():
                continue
            try:
                probes.append(VolumeProbe(root, subj_id, is_pos, n_slices, tile, halo))
